...
llik3 <- vector(length = max_it) # log likelihood of the EM iterations
...
for(it in 1:max_it) {
  ...
  if(it > 1){
    if(abs(llik3[it] - llik3[it - 1]) <= 0.1){
      break
    }
  }  
...
pi <- vector(length = M) # mixing coefficients
mu <- matrix(nrow=M, ncol=D) # conditional distributions
llik4 <- vector(length = max_it) # log likelihood of the EM iterations
...
for(it in 1:max_it) {
  ...
  # Your code here
    ...
    if(it > 1){
       if(abs(llik4[it] - llik4[it - 1]) <= 0.1){
         break
      }
    }  
...
pi <- vector(length = M) # mixing coefficients
mu <- matrix(nrow=M, ncol=D) # conditional distributions
llik2 <- vector(length = max_it) # log likelihood of the EM iterations
...
for(it in 1:max_it) {
  ...
  # Your code here
    ...
    if(it > 1){
       if(abs(llik2[it] - llik2[it - 1]) <= 0.1){
         break
      }
    }
...
```{r}
df = data.frame(llik2, llik3, llik4, x= 1:length(llik2))
```